...
const PropertiesPage = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3001/allbuyproperties?sortBy=${sortBy}`)
      .then(response => {
  ...
  const handleSeeMore = (propertyID) => {
    navigate(`/buyProperty/${propertyID}`);
  }
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {properties.map(property => (
          <div key={property.propertyID} className="col-md-4 mb-4" style={{ flexBasis: 'calc(33.33% - 20px)', margin: '10px', border: '1px solid #ccc', borderRadius: '5px', padding: '10px' }}>
            <Link to={`/buyProperty/${property.propertyID}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div className="card">
